...
def get_image_from_hubble(image_id):
  url = '{url}{image_id}'.format(url='http://hubblesite.org/api/v3/image/',image_id=image_id)
  ...
  filename = '{number}{format}'.format(number=image_id,format=os.path.splitext(file_url)[-1])
